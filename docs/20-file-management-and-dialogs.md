...
### 2-2. 열거 성능·메모리 규칙(09-15 · 사용자 "파일 탐색이 성능·메모리에 영향 없게 · 스레드 분리 · 사용 후 회수")

| 규칙 | 구현(`nexa-fs::lister` · `nexa-dlg`) |
|---|---|
| **UI 스레드는 디스크를 만지지 않는다** | 폴더 이동·새로 고침·지연 펼침·셰브론 프로브 전부 `ListHandle::start/probe`(요청마다 짧게 사는 스레드) · UI는 프레임 틱에서 `try_recv`만(프레임당 8개 상한 · 블록 0). 예외 = `is_dir()` 1회(이동 전 검증)와 `create_dir`(새 폴더). |
| **스트리밍** | `read_dir`를 256개 배치로 보내 첫 화면이 즉시 나온다 · 배치마다 정렬해 다시 그림 · 끝나면 `Done(총수)`. 프로브(빈 폴더 셰브론 억제)는 열거가 끝난 뒤 폴더마다 첫 일치에서 멈추는 판정 → `Probe(경로, 보여줄 자식?, 하위 폴더?)`로 도착 · 그때까지는 셰브론을 보여 준다(dir2 β 비동기 프로브). |
| **취소·회수** | `ListHandle`은 `Drop`에서 취소 플래그를 세운다 — 폴더를 바꾸거나 노드를 접거나 대화상자를 닫으면 핸들이 떨어져 워커가 다음 배치/프로브 경계에서 빠져나가고 미전송 배치는 채널과 함께 사라진다. 대화상자를 닫으면 `FilePicker`가 통째로 떨어지므로 목록·프로브·아이콘 변환 캐시가 전부 회수된다. 아이콘 워커는 **대화상자가 닫히면 즉시**(`FilePicker` Drop → `IconService::release_worker()`) · 그 밖엔 유휴 30초면 스스로 종료(다음 요청이 다시 만든다 · 종료와 `tx` 해제가 같은 잠금이라 유실 0) · 워커의 COM 아파트먼트는 스레드 수명과 같다(`ComApartment` · CoInitializeEx/CoUninitialize 짝). 아이콘 RGBA 캐시(≤512)는 프로세스에 남긴다(다음 열기 즉시 적중 · 확장자당 12ms 재조회 방지). **셸 첫 호출의 일회성 비용**(시스템 이미지 리스트·shell32 로드·COM = 프로세스에 GDI ≈+40 · 핸들 ≈+140)은 회수 대상이 아니다 — 열고 닫기를 반복해도 늘지 않는다(nexa-sql docs/37 §6-5 · `cargo test -p nexa-fs resource -- --ignored --nocapture`). |
| **메모리** | 항목은 `Entry`(이름·경로·크기·시각) 하나뿐 · 아이콘은 전역 512개 상한 · 프로브 결과는 폴더당 2비트 맵(이동 시 비움) · 지연 펼침은 접히면 로더와 항목을 버린다. 큰 폴더(10만 항목)는 그리드 재구성이 O(n)이라 배치마다 다시 그리는 비용이 쌓인다 — 가상화(F-3 FileList)가 다음 단계. |
| **느린 경로** | 네트워크·광학 드라이브도 같은 규칙(UI 비차단)이라 별도 예외가 필요 없다 · 프로브 생략 옵션(`skip_probe`)은 호출자가 고른다. |
