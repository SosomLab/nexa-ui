...
| **취소·회수** | `ListHandle`은 `Drop`에서 취소 플래그를 세운다 — 폴더를 바꾸거나 노드를 접거나 대화상자를 닫으면 핸들이 떨어져 워커가 다음 배치/프로브 경계에서 빠져나가고 미전송 배치는 채널과 함께 사라진다. 대화상자를 닫으면 `FilePicker`가 통째로 떨어지므로 목록·프로브·아이콘 변환 캐시가 전부 회수된다. 아이콘 워커는 유휴 30초면 스스로 종료(다음 요청이 다시 만든다 · 종료와 `tx` 해제가 같은 잠금이라 유실 0). |
