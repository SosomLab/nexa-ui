...
| 지점 | 값 |
|---|---|
| 줄 수 | 설정 `tabs.rows` = `single`(한 줄 · 넘치면 ◀ ▶ 스크롤 버튼 · 활성 탭 보이게 · 드래그로 순서 이동 · 기본) / `multi`(여러 줄로 접기 · Golden식) |
| **탭 툴팁** | 설정 `tabs.tooltip`(기본 켬) — 탭 위에 마우스가 머물면 **탭 설명 카드**(DBeaver 선례): 스크립트 경로 · 접속(프로필) · DBMS 종류 · URL(비밀번호 가림) · 스키마 · 프로젝트. 끄면 아무것도 안 뜬다. 내용은 앱이 `TabInfo`(제목 외 부가 줄 목록)로 주입 — 탭바는 표시만(DR-2) |
| 표시 규칙 | `draw_tooltip`(nexa-ctl 공용 · hover 1000ms 후 · 커서 아래 · 화면 안으로 클램프) · 여러 줄 = "라벨: 값" 정렬 · 드래그 중·팝업 열림 중엔 표시 안 함 |
| 닫기·핀·잠금 | dir2 그대로(× 호버 · 📌 고정 = 왼쪽 정렬 · 잠금 = 닫기 불가) · 가운데 클릭 = 닫기 |
| 세션 표시 | `session.mode = per-editor`면 탭에 접속 상태 ●(ok/warn) 마커 — [nexa-sql 24 `session.mode`](../../nexa-sql/docs/24-settings-and-vscode-analysis.md) |
