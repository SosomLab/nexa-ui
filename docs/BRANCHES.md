...
| 브랜치 | 생성 | 병합 | 커밋 | 요약 |
|---|---|---|---|---|
| feat/win-64-gutter-bookmark-zone | 2026-09-23 | 2026-09-23 → main(삭제) | 1 | 64차(win): `TextBox` 거터 배치 = **북마크 영역(띠·니모닉 상자) → 줄 번호 → 본문**(`bm_extra` 12 · 줄 변경 표시는 줄 번호 오른쪽 그대로 · 줄 번호 꺼도 북마크 영역 유지 · 글자만 안 그림) |
